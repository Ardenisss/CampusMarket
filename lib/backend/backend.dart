...
Future maybeCreateUser(User user) async {
  final userRecord = ListingsRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument = await ListingsRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createListingsRecordData(
    email: user.email ??
  ...
  currentUserDocument =
      ListingsRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createListingsRecordData(email: email));
}
